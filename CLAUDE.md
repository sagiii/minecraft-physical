...
### チャンネル競合ルール
- 同じチャンネル番号にINとOUTが両方設定された場合、**INが優先**（安全側に倒す）

## MQTTトピック設計

```
minecraft/ch/{n}/state   # Minecraftのレッドストーン状態 → M5Stack (OUTブロックが発行)
minecraft/ch/{n}/input   # M5StackのGPIO状態 → Minecraft  (INブロックが購読)
```

例: チャンネル1のOUTブロックがONになると `minecraft/ch/1/state` に `"1"` をパブリッシュ。
M5StackがこれをSubscribeしてLEDを光らせる。
